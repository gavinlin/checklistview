...
class DragLinearLayout: LinearLayout {
    ...
    /**
     * The currently dragged item, if [com.jmedeisis.draglinearlayout.DragLinearLayout.DragItem.detecting].
     */
    ...
    /**
     * Use with [com.jmedeisis.draglinearlayout.DragLinearLayout.setOnViewSwapListener]
     ...
     */
    inner class DragItem {
        var view: View? = null
        var startVisibility: Int = 0
    ...
    init {
        ...
        val a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.DragLinearLayout, 0, 0)
        try {
            scrollSensitiveHeight = a.getDimensionPixelSize(R.styleable.DragLinearLayout_scrollSensitiveHeight,
                    (DEFAULT_SCROLL_SENSITIVE_AREA_HEIGHT_DP * resources.displayMetrics.density + 0.5f).toInt())
        } finally {
            a.recycle()
        }

        nominalDistanceScaled = (NOMINAL_DISTANCE * resources.displayMetrics.density + 0.5f)
    }
    ...
    /**
     * See [com.jmedeisis.draglinearlayout.DragLinearLayout.OnViewSwapListener].
     */
     ...
     * [com.jmedeisis.draglinearlayout.DragLinearLayout.DragItem.detecting].
     */
    ...
    private fun onDragStop() {
        ...
        draggedItem.settleAnimation!!.addUpdateListener(ValueAnimator.AnimatorUpdateListener { animation ->
            ...
            draggedItem.setTotalOffset((animation.animatedValue as Float).toInt())
        ...
        if (isBelow || isAbove) {
            ...
            switchViewObserver.addOnPreDrawListener(object : OnPreDrawListener {
                override fun onPreDraw(): Boolean {
                    ...
                    val switchAnimator = ObjectAnimator.ofFloat(switchView, "y",
            ...
            val observer = draggedItem.view!!.getViewTreeObserver()
    ...
    override fun dispatchDraw(canvas: Canvas) {
        ...
        if (draggedItem.detecting && (draggedItem.dragging || draggedItem.settling())) {
            canvas.save()
            canvas.translate(0.toFloat(), draggedItem.totalDragOffset.toFloat())
            draggedItem.viewDrawable!!.draw(canvas)

            val left = draggedItem.viewDrawable!!.bounds.left
            val right = draggedItem.viewDrawable!!.bounds.right
            val top = draggedItem.viewDrawable!!.bounds.top
            val bottom = draggedItem.viewDrawable!!.bounds.bottom
    ...
    override fun onInterceptTouchEvent(event: MotionEvent): Boolean {
        when (MotionEventCompat.getActionMasked(event)) {
            MotionEvent.ACTION_DOWN -> {
                ...
                downY = MotionEventCompat.getY(event, 0).toInt()
            ...
            MotionEvent.ACTION_POINTER_UP -> {
                run {
                    ...
                    if (pointerId != activePointerId)
                        return@run// if active pointer, fall through and cancel!
                }
                run {
                    onTouchEnd()
    ...
    private fun getDragDrawable(view: View): BitmapDrawable {
        ...
        drawable.bounds = Rect(left, top, left + view.getWidth(), top + view.getHeight())
    ...
    companion object {
         ...
         */
        private fun smootherStep(edge1: Float, edge2: Float, step: Float): Float {
            val tempStep = Math.max(0f, Math.min((step - edge1) / (edge2 - edge1), 1f))
            return tempStep * tempStep * tempStep * (tempStep * (tempStep * 6 - 15) + 10)
        }
